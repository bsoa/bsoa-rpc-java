...
import io.bsoa.rpc.common.utils.*;
...
import static io.bsoa.rpc.common.json.JSON.getSerializeFields;
...
//                                value = colllection2Collection(clazz, genericType, (Collection) value);
...
//    }
    ...
    private static <T> T mapToObject(Map src, Class<T> dstClazz) {
        ...
        Class realClass = actualType != null ? ClassLoaderUtils.forName(actualType) : dstClazz;
        ...
        for (Field field : getSerializeFields(realClass)) {
            ...
            try {
                ...
                if (Collection.class.isAssignableFrom(fieldClazz)) {
                    ...
                    if (value instanceof Collection) {
                        value = colllection2Collection((Collection) value, fieldClazz, genericType);
                    } else if (value.getClass().isArray()) {
    ...
    private static <T> Collection<T> colllection2Collection(Collection src,
                                                            Class<? extends Collection> clazz, Class<T> genericType) {
    ...
    private static <T> T[] collectionToArray(Collection<T> src, Class<T> componentType) {
        return array2Array((T[]) src.toArray(), componentType);
    }
    ...
    public static <T> T deserializeByType(Object src, Class<T> clazz) {
        if (src == null) {
            ...
            if (clazz == Object.class) {
                return (T) colllection2Collection(list, list.getClass(), Object.class);
            } else if (Collection.class.isAssignableFrom(clazz)) {
                return (T) colllection2Collection(list, (Class<? extends Collection>) clazz, Object.class);
